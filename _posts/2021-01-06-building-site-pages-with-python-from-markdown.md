...
The "Academic Pages" template, updated and made available by Stuart Geiger, has a lot of nice features, including support for individual pages devoted to talks, publications, and teaching. Many of these are supported with helpers that assist in generating the markdown files that are needed to create individual pages, but there was not one available to create new entries for teaching experiences. Since I wanted to include these, I created a new tool that will help to create these pages using a `csv` file as a source. Using this tool, you can list out your teaching in a relatively simple list, then create all of the markdown files from that list. Following the model of the other generators, which are in jupyter notebooks, here's how the code looks:
